...
module.exports = (env = {}) => {
  ...
  const config = {
      ...
      ...isProd && {
        minimizer: [
          ...
          new OptimizeCSSAssetsPlugin({
            assetNameRegExp: /\.css\?v=([0-9a-zA-Z]+)mqcss$/g,
          }),
        ],
        splitChunks: {
          chunks: 'all',
    ...
    output: {
      ...
      ...isProd && {publicPath: '/test/react-test-01/'}
    },

    resolve: {
      alias: {
        '@components': path.join(__dirname, 'src', 'components'),
        '@connected': path.join(__dirname, 'src', 'connected'),
    ...
    plugins: [
      ...
      ...(!isLocal) ? [
        new MiniCssExtractPlugin({
          filename: `style.css${!isLocal ? '?v=[hash]mqcss' : ''}`,
          chunkFilename: `[name].css${!isLocal ? '?v=[hash]mqcss' : ''}`
        })
      ] : [],
